...
async function adminGetAgentsByAgency(agencyId) {
  const { data, error } = await _sb
    .from('agency_agents')
    .select('id, email, status, invited_at, accepted_at, user_id')
    .eq('agency_id', agencyId)
...
async function adminCreateAgent(agencyId, email) {
  const { data, error } = await _sb
    .from('agency_agents')
    .insert({ agency_id: agencyId, email, status: 'active', invited_at: new Date().toISOString() })
    .select()
    .single();
  return { data, error };
}
...
window.sbHelpers = {
  ...
  adminGetDocuments, adminUpdateDocument, adminDeleteDocument,
