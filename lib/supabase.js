...
async function getCertificateByCode(certCode) {
  const { data, error } = await _sb
    .from('certificates')
    .select('*, agencies(id, company_name, user_id), tenants(full_name, date_of_birth, nationality, permit_type, monthly_gross_salary, employer_name, job_role, employment_start_date, occupant_count, is_smoker, has_pets)')
...
window.sbHelpers = {
  ...
  upsertDocument, getDocumentSignedUrl,
};
