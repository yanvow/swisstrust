...
async function getAgencyAgentsDetailed(agencyId) {
  const { data, error } = await _sb
    .from('agency_agents')
...
async function createAgentWithPassword(agencyId, firstName, lastName, email, password) {
  const { data, error } = await _sb.functions.invoke('invite-agent', {
    body: { agencyId, email, firstName, lastName, password, mode: 'create' },
  });
  if (error) return { data: null, error: { message: error.message || 'Could not create agent' } };
  return { data, error: null };
}

async function getAgentActivityStats(agentUserId) {
  if (!agentUserId) return { totalViews: 0, lastActive: null, recentLogs: [] };
  const [{ count: totalViews }, { data: recentLogs }] = await Promise.all([
    _sb.from('document_access_logs')
      .select('id', { count: 'exact', head: true })
      .eq('viewer_user_id', agentUserId),
    _sb.from('document_access_logs')
      .select('accessed_at, certificates(property_address, property_city, cert_code)')
      .eq('viewer_user_id', agentUserId)
      .order('accessed_at', { ascending: false })
      .limit(5),
  ]);
  const lastActive = (recentLogs || [])[0]?.accessed_at || null;
  return { totalViews: totalViews || 0, lastActive, recentLogs: recentLogs || [] };
}
...
window.sbHelpers = {
  ...
  resolveAgency, getAgencyAgents, getAgencyAgentsDetailed, inviteAgent, removeAgent,
  createAgentWithPassword, getAgentActivityStats,
