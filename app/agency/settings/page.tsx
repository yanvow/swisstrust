...
export default function AgencySettingsPage() {
  const t = useT();
  const { userId, email, agencyId, isAdmin } = useAgency();
  ...
  return (
    <>
      ...
      <p className="text-gray-400 text-sm mt-1 mb-7">
        {t("Manage your agency account, payment methods, and billing.")}
      </p>
      ...
      {tab === "options" && (
        <OptionsTab userId={userId} email={email} agencyId={agencyId} />
      )}
      {tab === "usage" && <UsageTab agencyId={agencyId} />}
...
function OptionsTab({
  ...
  useEffect(() => {
    ...
    setPrefs({
      newCert: p["new-cert"] ?? true,
      agentChange: p["agent-change"] ?? true,
  ...
  function saveNotifs(next: typeof prefs) {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        "new-cert": next.newCert,
        "agent-change": next.agentChange,
  ...
  async function confirmClearInfo() {
    if (!agencyId) {
      alert(t("Agency profile not found."));
      return;
    }
    if (
      !confirm(
        t(
          "Clear your agency profile (name, address, contact email)? This cannot be undone.",
        ),
      )
    )
      return;
    ...
    const { error } = await sb
      .from("agencies")
      .update({ company_name: "", address: "", contact_email: "" })
      .eq("id", agencyId);
    if (error) {
      alert(t("Error") + ": " + error.message);
      return;
    }
    alert(t("Agency profile information has been cleared."));
  }

  async function confirmRemoveAllAgents() {
    if (!agencyId) {
      alert(t("Agency profile not found."));
      return;
    }
    if (
      !confirm(
        t(
          "Remove ALL agents from your agency? They will immediately lose access. This cannot be undone.",
        ),
      )
    )
      return;
    ...
    alert(t("All agents have been removed."));
  }
  ...
  async function confirmDeleteAccount() {
    if (
      !confirm(
        t(
          "PERMANENTLY delete your Checks agency account?\n\nAll agents, certificates, and data will be removed. This cannot be undone.",
        ),
      )
    )
      return;
    ...
    <>
      <fieldset className="fieldset">
        ...
        <div className="flex flex-col gap-5">
          <Row
            title={t("Clear agency profile information")}
            desc={t(
              "Resets your company name, address, and contact email. Your agents and certificate inbox are not affected.",
            )}
            action={
              <button
          ...
          <Row
            title={t("Remove all agents")}
            desc={t(
              "Removes all team members from your agency. They will lose access immediately. Their individual accounts are not deleted.",
            )}
            action={
              <button
                className="btn btn-outline btn-sm"
                style={{ color: "var(--amber)", borderColor: "var(--amber)" }}
                onClick={confirmRemoveAllAgents}
              >
                {t("Remove all agents")}
              </button>
            }
      ...
      <fieldset className="fieldset" style={{ marginTop: 24 }}>
        ...
        <div className="flex flex-col gap-4">
          <Toggle
            title={t("New certificate in inbox")}
            desc={t("Receive an email when a tenant sends a certificate to your agency inbox.")}
            checked={prefs.newCert}
            onChange={(v) => saveNotifs({ ...prefs, newCert: v })}
          />
          <Toggle
            title={t("Agent joined or left")}
            desc={t(
              "Get notified when an agent accepts an invitation or is removed from your team.",
            )}
            checked={prefs.agentChange}
            onChange={(v) => saveNotifs({ ...prefs, agentChange: v })}
          />
          <Toggle
            title={t("Weekly digest")}
            desc={t("A weekly summary of inbox activity, agent usage, and certificate reviews.")}
      ...
      <fieldset className="fieldset" style={{ marginTop: 24, borderColor: "#FECACA" }}>
        ...
        <Row
          ...
          desc={t(
            "Permanently deletes your Checks agency account and all associated data including agents and certificates. This action cannot be undone.",
          )}
          action={
            <button
              className="btn btn-outline btn-sm"
...
function UsageTab({ agencyId }: { agencyId: string }) {
  const t = useT();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [viewsRes, agentsRes] = await Promise.all([
        sb
          .from("certificate_inbox")
    ...
    <>
      <div className="flex items-center justify-between gap-4 px-5 py-4 bg-gray-100 rounded-[2px] mb-6 flex-wrap">
        <div>
          ...
          <div className="text-xs text-gray-400 mt-0.5">
            {t("5 full dossier views/month · 1 agent seat · View-only (no listings or analytics)")}
          </div>
        </div>
        <button className="btn btn-primary btn-sm" disabled title={t("Coming soon")}>
          {t("Upgrade to Starter")}
        </button>
      </div>
      ...
      <fieldset className="fieldset">
        ...
        {viewsCount === null ? (
          ...
          <UsageBar
            label={t("Full dossier views")}
            used={viewsCount}
            limit={5}
            sub={t("Monthly allowance on Free Viewer. Upgrade to Starter for 30/month.")}
          />
        )}
      </fieldset>
      ...
      <fieldset className="fieldset" style={{ marginTop: 20 }}>
        ...
        {agentsCount === null ? (
          ...
          <UsageBar
            ...
            used={agentsCount}
            limit={1}
            sub={t("Active team members with inbox access. Upgrade to Starter for 3 seats.")}
          />
        )}
      </fieldset>
    </>
