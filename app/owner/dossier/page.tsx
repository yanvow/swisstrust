...
export default function OwnerDossierPage() {
  const t = useT();
  const { userId, email, displayName } = useOwner();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: cert, error } = await sb
        .from("certificates")
        .select(
          "*, tenants(full_name, date_of_birth, nationality, permit_type, monthly_gross_salary, employer_name, job_role, employment_start_date, occupant_count, is_smoker, has_pets)",
        )
        .eq("cert_code", code)
        .eq("is_active", true)
        .single<Cert>();
      ...
      const isDirected =
        cert.mode === "directed" &&
        !!cert.owner_email &&
        cert.owner_email.toLowerCase() === email.toLowerCase();
      if (!isDirected) {
      ...
      const [{ data: docs }, { data: logs }] = await Promise.all([
        sb.from("documents").select("*").eq("tenant_id", cert.tenant_id).order("created_at"),
        sb
  ...
  if (state.kind === "error") {
    return (
      <div className="py-12 text-center">
        ...
        <Link href="/owner/dashboard" className="btn btn-outline btn-sm mt-4 inline-block">
          ← {t("Back to dashboard")}
        </Link>
      </div>
    );
  ...
  return (
    <>
      <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
        <div>
          ...
          <div className="text-xs text-gray-400 mt-1">
            {t("Certificate")}: <span className="font-mono font-bold">{cert.cert_code}</span> ·{" "}
            {t("Issued")} {issued}
          </div>
        </div>
      ...
      <div className="alert alert-success mb-6">
        <span>✓</span>
        <span>
          {t("This access has been logged")} — {accessTime} {t("by")} {displayName}.{" "}
      ...
      <div className="mt-2">
        <Link href="/owner/dashboard" className="btn btn-outline">
          ← {t("Back to dashboard")}
        </Link>
      </div>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-200 gap-3">
              <span className="font-semibold text-[0.9375rem] truncate">{preview.fileName}</span>
              <button
                onClick={closePreview}
